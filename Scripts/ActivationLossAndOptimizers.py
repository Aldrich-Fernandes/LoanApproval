...
class Sigmoid:
    def forward(self, inputs):
        self.inputs = list(map(lambda z: z[0], inputs))
        self.outputs = [1 / (np.exp(-val) + 1) for val in self.inputs]
...
class OptimizerSGD:
    ...
    def UpdateParameters(self, layer): ### Issue - result keeps increasing to 1 until crash
        ...
        layer.weights = [[a+b for a, b in zip(layer.weights[x], AdjustedDWeight[x])] 
